...
function run(address[] memory tokens, address[] memory whitelist, address[] memory priceFeeds, uint256[] memory minCollateralRatios, address feeReceiver) public returns (VaultManager vaultManager, StabilskiToken stabilskiToken, USDPLNOracle usdPlnOracle, CollateralManager collateralManager) {
    // Deploy contracts
    vm.startBroadcast();
...
if(block.chainid == vm.envUint("ETH_SEPOLIA_CHAINID")) {
    ...
    vaultManager = new VaultManager(address(usdPlnOracle), address(stabilskiToken), address(collateralManager), tokens[0], feeReceiver);
...
if(block.chainid == vm.envUint("ARBITRUM_TESTNET_CHAINID")) {
    ...
    collateralManager = new CollateralManager(tokens, priceFeeds, minCollateralRatios);
    vaultManager = new VaultManager(address(usdPlnOracle), address(stabilskiToken), address(collateralManager), address(0), feeReceiver);
